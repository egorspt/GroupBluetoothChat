...
public class BluetoothService implements Serializable {
    ...
    ArrayList<ActiveConnection> listOfConnectio = new ArrayList<>();
    public static HashMap<String, ConnectedThread> listOfConnection = new HashMap<>();
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice
        ...
        mConnectedThread = new ConnectedThread(socket, socketType, device.getAddress());
     ...
     */
    public void write(byte[] out, int view) {
    ...
    public void write(byte[] out, int view, String address, int state) {
        // Create temporary object
        ConnectedThread r;
        // Synchronize a copy of the ConnectedThread
        synchronized (this) {
            ...
            //r = mConnectedThread;
            r = listOfConnection.get(address);
        }
        // Perform the write unsynchronized
        r.write(out, view);
    }
    ...
    private class ConnectedThread extends Thread implements Serializable {
        ...
        public void run() {
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    ...
                    if (pos > 300)
                        mHandler.obtainMessage(Constants.MESSAGE_READ_IMAGE, pos, -1, w)
                                .sendToTarget();
                    else
                        mHandler.obtainMessage(Constants.MESSAGE_READ, pos, -1, w)
                                .sendToTarget();
                } catch (IOException e) {
         ...
         */
        public void write(byte[] buffer, int view) {
            try {
                mmOutStream.write(buffer);

                if (view == Constants.TEXT)
                    mHandler.obtainMessage(Constants.MESSAGE_WRITE, -1, -1, buffer)
                            .sendToTarget();
                else
                    mHandler.obtainMessage(Constants.MESSAGE_WRITE_IMAGE, -1, -1, buffer)
                            .sendToTarget();
            } catch (IOException e) {
